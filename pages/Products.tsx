...
const Products: React.FC = () => {
    ...
    return (
        <div className="space-y-6">
            ...
            {isBulkUpdateModalOpen && <BulkPriceUpdateModal 
                allProducts={Array.from(new Map([...products, ...serverSearchResults].map(p => [p.id, p])).values())} 
                suppliers={suppliers} 
                onClose={() => setIsBulkUpdateModalOpen(false)} 
                onBulkUpdate={handleBulkUpdate} 
            />}
            {isBulkLocationUpdateModalOpen && <BulkLocationUpdateModal 
                allProducts={Array.from(new Map([...products, ...serverSearchResults].map(p => [p.id, p])).values())} 
                purchases={purchases} 
                suppliers={suppliers} 
                onClose={() => setIsBulkLocationUpdateModalOpen(false)} 
                onBulkUpdate={handleBulkLocationUpdate} 
            />}
