...
const POS: React.FC = () => {
    ...
    const fetchData = useCallback(async (silent = false, retryCount = 0) => {
        ...
        console.log('POS: Starting fetchData for user:', user.id);
        if (!silent) {
            setLoading(true);
            setError(null);
        }
        ...
        try {
            ...
            const [sess, usrs, sls] = await Promise.all([
                getCashSessions(user.id), getUsers(), getSales(user.id)
            ]);
