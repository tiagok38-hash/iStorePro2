...
import { SpinnerIcon, SmartphoneIcon, TagIcon, UserIcon, CubeIcon, ChartBarIcon, CurrencyDollarIcon, ClockIcon, CreditCardIcon, PlusIcon, DeviceExchangeIcon, ArchiveBoxIcon, UsersIcon } from '../components/icons.tsx';
    ...
    return (
        <div className={`p-6 rounded-[32px] border border-gray-100 shadow-xl shadow-gray-300/80 flex flex-col h-full ${className || 'bg-surface'}`}>
            <div className="flex items-center gap-2 mb-3">
                <TagIcon className="h-4 w-4 text-danger" />
                <h3 className="text-sm font-black text-secondary uppercase tracking-wider">Últimos Produtos Vendidos</h3>
            </div>
            {soldItems.length === 0 ? (
...
const RecentAddedProductsCard: React.FC<{ products: Product[]; className?: string }> = React.memo(({ products, className }) => {
    return (
        <div className={`p-4 sm:p-6 rounded-[32px] border border-gray-100 shadow-xl shadow-gray-300/80 flex flex-col h-full ${className || 'bg-surface'}`}>
            <div className="flex items-center gap-2 mb-3">
                <PlusIcon className="h-4 w-4 text-success" />
                <h3 className="text-sm font-black text-secondary uppercase tracking-wider">Ultimos produtos lancados no estoque</h3>
            </div>
            {products.length === 0 ? (
    ...
    return (
        <div className={`p-4 sm:p-6 rounded-[32px] border border-gray-100 shadow-xl shadow-gray-300/80 flex flex-col h-full ${className || 'bg-surface'}`}>
            <div className="flex justify-between items-center mb-3">
                <div className="flex items-center gap-2">
                    <DeviceExchangeIcon className="h-4 w-4 text-orange-600" />
...
const StockStatsCard: React.FC<{ products: Product[]; className?: string }> = React.memo(({ products, className }) => {
    ...
    return (
        <div className={`p-6 rounded-[32px] border border-gray-100 shadow-xl shadow-gray-300/80 bg-surface ${className}`}>
            <div className="flex justify-between items-center mb-4">
                <div className="flex items-center gap-2">
                    <ArchiveBoxIcon className="h-4 w-4 text-secondary" />
