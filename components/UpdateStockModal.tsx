...
const UpdateStockModal: React.FC<UpdateStockModalProps> = ({ product, onClose, onSave }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-60 flex justify-center items-center z-50">
      <div className="bg-surface rounded-3xl shadow-xl p-6 w-full max-w-md mx-4">
        ...
        <div className="flex items-center justify-center gap-4 my-4">
          ...
          <div>
            ...
            <div className="flex items-center gap-2">
              ...
              <input type="number" value={adjustment} onChange={(e) => setAdjustment(parseInt(e.target.value) || 0)} className="w-20 text-center p-2 border rounded-xl bg-transparent border-border text-4xl font-bold" />
        ...
        <div className="flex justify-end space-x-4 mt-8">
          <button
            type="button"
            onClick={onClose}
            className="px-4 py-2 bg-danger text-white rounded-xl hover:bg-danger/90"
          >
            Cancelar
          </button>
          <button
            type="button"
            onClick={handleSave}
            className="px-4 py-2 bg-success text-white rounded-xl hover:bg-success/90 disabled:bg-muted disabled:cursor-not-allowed"
