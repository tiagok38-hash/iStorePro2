                                ...
                                <tr key={session.id} className="hover:bg-gray-50/50 transition-colors">
                                    ...
                                    <td className="px-6 py-4">
                                        <div className="flex items-center justify-center gap-1">
                                            ...
                                            {session.status === 'fechado' && toDateValue(session.openTime) === toDateValue() && <button onClick={() => onViewDetails(session)} className="p-1.5 text-blue-600 hover:bg-blue-50 rounded" title="Ver detalhes para reabrir"><ArrowPathRoundedSquareIcon className="h-4 w-4" /></button>}
                                        </div>
                                    </td>
                                </tr>
                            ))}
                        ...
                        <div key={session.id} className="p-1.5 glass-panel bg-white/40 hover:bg-white/60 transition-colors border border-white/20 mb-2 rounded-xl shadow-sm">
                            ...
                            <div className="flex items-center justify-between gap-2">
                                ...
                                <div className="flex items-center gap-1">
                                    ...
                                    {session.status === 'fechado' && toDateValue(session.openTime) === toDateValue() && <button onClick={() => onViewDetails(session)} className="p-1 px-2 bg-blue-50 text-blue-600 rounded-xl border border-blue-100 transition-colors" title="Ver detalhes para reabrir"><ArrowPathRoundedSquareIcon className="h-3.5 w-3.5" /></button>}
                                </div>
                            </div>
                        </div>
                    ))}
