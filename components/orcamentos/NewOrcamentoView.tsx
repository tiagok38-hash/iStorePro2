...
import {
    Product, Customer, User, Orcamento, PermissionProfile,
    Brand, Category, ProductModel, Grade, GradeValue,
    Supplier, ReceiptTermParameter, PaymentMethodParameter
} from '../../types.ts';
import {
    formatCurrency, getProducts, getCustomers, getUsers,
    getSuppliers, getPermissionProfiles, getBrands, getCategories,
    getProductModels, getGrades, getGradeValues, getReceiptTerms, getPaymentMethods
} from '../../services/mockApi.ts';
import {
    PlusIcon, MinusIcon, EditIcon, ShoppingCartIcon, CalculatorIcon, CreditCardIcon, SpinnerIcon,
...
import { useOrcamentoForm } from '../../hooks/useOrcamentoForm.ts';
import { getPaymentIcon } from '../pos/utils.tsx';

interface NewOrcamentoViewProps {
    onCancel: () => void;
    onOrcamentoSaved: (orcamento: Orcamento) => void;
    customers: Customer[];
    users: User[];
    products: Product[];
    suppliers: Supplier[];
...
export const NewOrcamentoViewInner: React.FC<NewOrcamentoViewProps> = (props) => {
    const {
    ...
    const { state, actions, refs } = useOrcamentoForm(props);
    ...
    const {
        setOrcamentoDate, setSelectedCustomerId, setSelectedSalespersonId, setProductSearch,
        setSearchQuantity,
    ...
    const inputClasses = "w-full px-4 border rounded-xl bg-white border-gray-300 text-sm h-12 focus:ring-2 focus:ring-orange-500/20 outline-none transition-all font-bold text-gray-800 shadow-sm box-border";
            ...
            <div className="glass-card rounded-3xl shadow-lg overflow-hidden animate-slide-up">
                    ...
                    <section>
                        <div className="flex items-center justify-between mb-1 sm:mb-2 pb-1.5 sm:pb-2 border-b border-gray-100">
                            <h3 className="flex items-center gap-2 font-bold text-gray-800 uppercase text-[10px] sm:text-xs tracking-widest">
                                <EditIcon className="h-3.5 w-3.5 text-orange-500" />
                                {props.orcamentoToEdit ? `Dados do Orçamento #${props.orcamentoToEdit.numero}` : 'Dados do Orçamento'}
                            </h3>
                            <button
                                type="button"
                                onClick={handleCancelOrçamento}
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-12 gap-2 sm:gap-4 bg-white/40 p-1 rounded-xl">
                            ...
                            <div className="md:col-span-7 flex flex-col justify-end">
                                ...
                                <div className="flex items-stretch gap-2 h-10 sm:h-12">
                                    ...
                                    <button type="button" onClick={() => { setCustomerToEdit(null); setIsCustomerModalOpen(true); }} className="w-10 sm:w-12 h-full bg-orange-100 text-orange-500 rounded-xl hover:bg-orange-500 hover:text-white transition-all flex items-center justify-center border border-orange-500/20 shadow-sm flex-shrink-0"><PlusIcon className="h-5 w-5" /></button>
                                </div>
                            </div>
                    ...
                    <fieldset disabled={!selectedCustomerId || !selectedSalespersonId} className="space-y-4 disabled:opacity-50 transition-opacity">
                        <section>
                            <div className="flex items-center justify-between mb-2">
                                <h3 className="flex items-center gap-2 font-bold text-gray-800 uppercase text-[10px] sm:text-xs tracking-widest"><ShoppingCartIcon className="h-4 w-4 text-orange-500" /> Carrinho</h3>
                            </div>
                                    ...
                                    {filteredProducts.length > 0 && (
                                        <div className="absolute z-30 w-full bg-white border border-gray-200 rounded-xl mt-1 shadow-xl max-h-60 overflow-y-auto overflow-x-hidden">
                                            {filteredProducts.map(p => (
                                                <div key={p.id} className="w-full p-2.5 hover:bg-orange-100 text-left transition-colors border-b last:border-0 border-gray-50 flex justify-between items-center group">
                                                    <div className="flex flex-col gap-1 cursor-pointer flex-grow" onClick={() => { handleAddToCart(p); setProductSearch(''); }}>
                                                        <p className="font-bold text-gray-900 text-base md:text-lg group-hover:text-orange-500 transition-colors leading-tight">{p.model}</p>
                                                        <div className="flex flex-wrap items-center gap-x-2 gap-y-1">
                                                            <span className="px-2 py-0.5 bg-orange-50 text-orange-700 rounded-xl text-[11px] md:text-xs font-black border border-orange-100 uppercase tracking-tighter">
                                                                Estoque: {p.stock}
                                                            </span>
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-5 gap-4">
                            <section className="lg:col-span-2">
                                <h3 className="flex items-center gap-2 font-bold text-gray-800 mb-2 pb-1 border-b border-gray-100 uppercase text-[10px] sm:text-xs tracking-widest"><CalculatorIcon className="h-4 w-4 text-orange-500" /> Financeiro</h3>
                                ...
                                <div className="glass-panel bg-white/40 rounded-3xl p-3 sm:p-4 space-y-2 sm:space-y-3 shadow-sm border border-white/20">
                                    ...
                                    <div className="flex justify-between items-center pt-2 mt-1 border-t border-gray-100">
                                        <span className="text-base sm:text-lg uppercase font-black tracking-widest text-orange-500 leading-none">Total</span>
                            ...
                            <section className="lg:col-span-3">
                                <h3 className="flex items-center gap-2 font-bold text-gray-800 mb-2 pb-1 border-b border-gray-100 uppercase text-[10px] sm:text-xs tracking-widest"><CreditCardIcon className="h-4 w-4 text-orange-500" /> Pagamento</h3>
                                    ...
                                    <div className="grid grid-cols-3 sm:grid-cols-4 lg:grid-cols-5 gap-1.5 sm:gap-2">
                                        {paymentButtons.map(({ label, icon }) => (
                                            <button
                                                ...
                                                className="w-full p-1.5 py-2 rounded-xl border border-gray-200 hover:border-orange-500 hover:bg-orange-100 transition-all flex flex-col items-center justify-center gap-1 group disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:bg-transparent disabled:hover:border-gray-200 min-h-[58px] sm:min-h-[64px]"
                                            >
                                                <div className="p-1 rounded bg-gray-50 group-hover:bg-white group-disabled:bg-gray-50 transition-colors">{React.cloneElement(icon as React.ReactElement, { className: "h-3.5 w-3.5 sm:h-5 sm:w-5 text-gray-500 group-hover:text-orange-500 group-disabled:text-gray-400" })}</div>
                                                <span className="text-[7.5px] sm:text-[9px] font-black uppercase text-gray-600 group-hover:text-orange-500 group-disabled:text-gray-400 text-center leading-[1.1] sm:leading-tight line-clamp-2">{label}</span>
                                            </button>
                                        ))}
                                    </div>

                                    {paymentInput && (
                                        <div className="p-2 bg-orange-100 border border-orange-500/30 rounded-xl flex items-end gap-2 animate-slide-down">
                                            <div className="flex-grow">
                                                <label className="text-[9px] font-bold text-orange-600 mb-1 block">
                                            ...
                                            {paymentMethods.find(m => m.name === paymentInput.method)?.allowInternalNotes && (
                                                <div className="flex-grow">
                                                    <label className="text-[9px] font-bold text-orange-600 mb-1 block">Obs. Interna (Opcional)</label>
                                                    <input
                                                        type="text"
                                                        value={paymentInput.internalNote || ''}
                                                        onChange={e => setPaymentInput(p => p ? { ...p, internalNote: e.target.value } : null)}
                                                        className="w-full h-10 px-3 bg-white border border-gray-200 rounded-xl text-xs font-bold text-gray-700 focus:ring-4 focus:ring-orange-500/10 outline-none transition-all"
                                                        placeholder="Anotação..."
                                                    />
                                                </div>
                                            )}
                                            <button onClick={handleConfirmPayment} className="px-3 h-10 bg-orange-500 text-white rounded-xl font-bold shadow-sm text-xs">OK</button>
                                            ...
                                            <tbody className="divide-y divide-gray-50">
                                                ...
                                                {payments.length > 0 && (
                                                    <tr className="bg-orange-100/30">
                                                        <td className="px-3 py-1.5 font-bold uppercase text-[9px] text-orange-500">Pago</td>
                                                        <td className="px-3 py-1.5 text-right font-black text-orange-500">{formatCurrency(totalPaid)}</td>
                                                        <td></td>
                                                    </tr>
                                                )}
                                            </tbody>
                ...
                <div className="p-2 md:p-4 bg-gray-50/30 border-t border-white/20 space-y-3 sm:space-y-4">
                            ...
                            <div className="relative">
                                <select
                                    ...
                                    className={`w-full pl-4 pr-10 border rounded-xl text-sm h-10 sm:h-12 focus:ring-2 outline-none transition-all font-bold shadow-sm appearance-none ${!warrantyTerm
                                        ...
                                        : 'bg-white border-gray-300 focus:ring-orange-500/20 text-gray-800'
                                        }`}
                                >
                    ...
                    <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-end gap-2 sm:gap-3 pt-3 border-t border-gray-100 sm:border-0 sm:pt-0">
                        ...
                        <button onClick={() => handleSave('Finalizada')} disabled={cart.length === 0 || balance > 0.01 || !warrantyTerm || isSaving} className="h-12 sm:h-12 px-8 sm:px-14 bg-orange-500 text-white rounded-xl font-black uppercase text-xs sm:text-sm tracking-widest shadow-lg hover:bg-orange-600 transition-all disabled:bg-gray-200 flex items-center justify-center gap-2">
                            {isSaving ? (
                                <>
                    ...
                    <div className="bg-white rounded-3xl shadow-2xl w-full max-w-sm overflow-hidden animate-scale-in">
                        ...
                        <div className="p-4 space-y-2">
                            {variationModalConfig.variations.map((variation, idx) => (
                                ...
                                >
                                    <span>{variation}</span>
                                    <ChevronRightIcon className="h-5 w-5 text-gray-300 group-hover:text-orange-500 transition-colors" />
                                </button>
                            ))}
                        </div>
                    </div>
                    ...
                    <div className="bg-white p-8 rounded-3xl shadow-2xl flex flex-col items-center gap-6 max-w-xs w-full text-center animate-scale-in">
                        <div className="relative">
                            ...
                            <div className="absolute inset-0 flex items-center justify-center">
                                <ShoppingCartIcon className="h-6 w-6 text-orange-500 animate-pulse" />
                            </div>
                        </div>
                        <div className="space-y-2">
                            <h3 className="text-xl font-black text-gray-900 uppercase tracking-tight">Processando Orçamento</h3>
                            <p className="text-sm text-gray-500 font-medium leading-relaxed">
                                Estamos atualizando o estoque e registrando seu histórico. <br />
                                <span className="text-orange-500 font-bold mt-1 block">Por favor, aguarde...</span>
                            </p>
                        </div>
                    </div>
                </div>
                        ...
                        try {
                            ...
                            if (customerToEdit) {
                                if (props.onUpdateCustomer) {
                                    ...
                                    showToast('Cliente atualizado com sucesso!', 'orange-500');
                                } else {
                                    console.warn('onUpdateCustomer prop missing in NewOrcamentoView');
                                }
                            } else {
                                // Create Mode
                                const nc = await onAddNewCustomer(customerPayload);
                                if (nc) setSelectedCustomerId(nc.id);
                                showToast('Cliente cadastrado com sucesso!', 'orange-500');
                            }

                            setIsCustomerModalOpen(false);
                            setCustomerToEdit(null);
                        } catch (error: any) {
                            console.error('NewOrcamentoView: Error saving customer:', error);
                        ...
                        <div className="p-4 max-h-[60vh] overflow-y-auto space-y-3 custom-scrollbar">
                            {matchingUnits.map(unit => (
                                <button
                                    ...
                                    className="w-full p-4 bg-white border border-gray-200 rounded-2xl hover:border-orange-500 hover:bg-orange-100/30 transition-all flex flex-col gap-2 group text-left shadow-sm"
                                >
                                    <div className="flex justify-between items-start">
                                        <span className="font-black text-gray-900 text-sm group-hover:text-orange-500 transition-colors">{unit.model}</span>
                                        <span className={`text-[10px] font-black px-2 py-0.5 rounded-full uppercase tracking-tighter shadow-sm ${unit.condition === 'Novo' ? 'bg-orange-500/10 text-orange-500 border border-orange-500/20' :
                                    ...
                                    <div className="flex items-center justify-between">
                                        ...
                                        <div className="flex items-center bg-white border border-gray-300 rounded-xl h-11 w-36 shadow-md overflow-hidden shrink-0">
                                            ...
                                            <button
                                                ...
                                                className="h-full w-11 flex items-center justify-center text-gray-500 hover:bg-orange-50 hover:text-emerald-600 transition-colors border-l border-gray-100"
                                            >
                            ...
                            <div className="space-y-2 pt-3 border-t border-gray-100">
                                ...
                                <div className="grid grid-cols-3 gap-2">
                                    ...
                                    <div
                                        ...
                                        className={`p-2 rounded-xl border-2 flex flex-col items-center justify-center transition-all cursor-pointer hover:bg-orange-50 ${selectedPriceType === 'sale' ? 'bg-orange-50 border-emerald-500 shadow-md transform scale-[1.02]' : 'bg-gray-50 border-gray-100 grayscale-[0.5] opacity-60'}`}
                                    >
                                        <span className="text-[8px] font-bold uppercase mb-0.5 opacity-60">Orçamento</span>
...
export const NewOrcamentoViewInnerMemoized = React.memo(NewOrcamentoViewInner);

export default function NewOrcamentoView(props: { onCancel: () => void; onSaved: (orcamento: Orcamento) => void; orcamentoToEdit?: Orcamento | null }) {
    ...
    useEffect(() => {
        ...
        async function fetchAll() {
            try {
                ...
                console.error('Failed to load data for NewOrcamentoView', err);
            } finally {
    ...
    if (loading || !data) {
        return (
            <div className="flex flex-col items-center justify-center p-12 text-center text-gray-500 min-h-[500px]">
                <SpinnerIcon className="h-8 w-8 text-orange-500 animate-spin mb-4" />
    ...
    return (
        <NewOrcamentoViewInnerMemoized
            {...data}
            {...props}
            onOrcamentoSaved={props.onSaved}
            onAddProduct={async () => null}
            onUpdateCustomer={async () => null}
        />
    );
}
