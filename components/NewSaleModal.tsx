...
const NewSaleModal: React.FC<NewSaleModalProps> = (props) => {
    ...
    return createPortal(
        <div className="fixed inset-0 z-[60] flex items-end lg:items-center justify-center bg-black/60 backdrop-blur-sm p-0 lg:p-4 animate-fade-in overflow-hidden">
            <div className="bg-white rounded-none lg:rounded-2xl shadow-2xl w-full max-w-6xl h-[100dvh] lg:h-auto lg:max-h-[95vh] flex flex-col relative animate-scale-in" style={{ paddingTop: 'env(safe-area-inset-top, 0px)' }}>
                <div className="flex items-center justify-between p-4 border-b border-gray-100 shrink-0">
                    <h2 className="text-xl font-bold text-gray-800">
                        {props.saleToEdit ? `Editar Venda #${props.saleToEdit.id}` : "Nova Venda"}
                    </h2>
                ...
                <div className="flex-grow overflow-y-auto custom-scrollbar p-2 pb-safe-bottom" style={{ paddingBottom: 'calc(env(safe-area-inset-bottom, 0px) + 80px)' }}>
                    <NewSaleView
                        {...props}
